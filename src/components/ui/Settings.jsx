  ...
  return (
    <div className="min-h-screen bg-stone-800 text-white p-8">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Audio Settings */}
        <div className="bg-stone-900 border-2 border-stone-700 p-6 mb-6">
        ...
        {/* Danger Zone */}
        <div className="bg-red-950 border-2 border-red-800 p-6 mb-6">
          <h2 className="text-2xl font-bold mb-2 text-red-400">Danger Zone</h2>
          <p className="mb-4 text-stone-300">This action cannot be undone!</p>

          {!showResetConfirm ? (
            <button
              onClick={() => setShowResetConfirm(true)}
              className="px-6 py-2 bg-red-800 border-2 border-red-700 hover:bg-red-700 font-bold"
            >
              Reset All Progress
            </button>
          ) : (
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                <button
                  onClick={handleReset}
                  className="px-6 py-2 bg-red-600 border-2 border-red-500 hover:bg-red-500 font-bold"
                >
                  Yes, Reset Everything
                </button>
                <button
                  onClick={() => setShowResetConfirm(false)}
                  className="px-6 py-2 bg-stone-700 border-2 border-stone-600 hover:bg-stone-600 font-bold"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="flex gap-4">
          <button
            onClick={handleSave}
            className="px-8 py-3 bg-emerald-600 border-2 border-emerald-500 hover:bg-emerald-500 font-bold"
          >
            Save Settings
          </button>
          <button
            onClick={() => setPhase('menu')}
            className="px-8 py-3 bg-stone-700 border-2 border-stone-600 hover:bg-stone-600 font-bold"
          >
            Back to Menu
          </button>
        </div>
